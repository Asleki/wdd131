        /* === Consolidated JavaScript from app_core.js, main_script.js, standalone_header.js, movies.js === */
        ...
        // Functions for header search and dropdown (from standalone_header.js)
        ...
        function updateAccountDropdown() {
            ...
            if (userLoggedIn) {
                ...
                playlistLink.href = "playlist.html"; // Link to playlist page
                playlistLink.innerHTML = `<i class="fas fa-list-ul"></i> Playlist`;
                accountDropdown.appendChild(playlistLink);

                const favoriteLink = document.createElement("a");
                favoriteLink.href = "favorite.html"; // Link to favorite page
                favoriteLink.innerHTML = `<i class="fas fa-heart"></i> Favorite`;
                accountDropdown.appendChild(favoriteLink);

                // Settings with conceptual sub-items
                const settingsLink = document.createElement("a");
                settingsLink.href = "settings.html"; // Link to settings page
                settingsLink.innerHTML = `<i class="fas fa-cog"></i> Settings`;
                accountDropdown.appendChild(settingsLink);
                // Note: Actual sub-menus for settings (Profile Picture, Change Password, Close Account)
                // would be handled on the settings.html page or via a more complex dropdown UI if needed.
                // For this consolidation, the link just goes to settings.html.
                ...
                favoriteLink.addEventListener("click", (e) => {
                    e.preventDefault();
                    showMessageBox("You can't view playlist without logging in.", "fas fa-info-circle blue-icon"); // Same message as playlist
                });
                accountDropdown.appendChild(favoriteLink);
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // --- movies.js specific logic (adapted from tv_shows.js) ---
            const moviesListContainer = document.getElementById('moviesList');

            if (typeof allShows === 'undefined') {
                console.error("allShows data is not available. Please ensure app_core.js data is correctly defined.");
            ...
            // Filter for movies: A show is considered a movie if its 'seasons' property is "1 (Film)"
            const movies = allShows.filter(show => show.seasons === "1 (Film)");
            ...
            if (moviesListContainer) {
                ...
                if (movies.length > 0) {
                    movies.forEach(movie => {
                        const movieCardElement = createShowCardHTML(movie); // Use the common function
